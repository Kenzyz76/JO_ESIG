...
def effacer_cadre_milieu():
    for child in fenetre.winfo_children():
        if isinstance(child, tk.Frame) and tk.Frame != cadre_gauche and tk.Frame != cadre_droit and tk.Frame != cadre_bouton_retour1 :
            child.destroy()
            

def effacer_label():
    for child in fenetre.winfo_children():
        if isinstance(child, tk.Label) or isinstance(child, tk.Canvas):
            child.destroy()
...
def afficher_plans():
    ...
    canvas.pack()
    original_img = Image.open("600_sc_59aa8-les-sites-des-preuves-olympiques-paris-1.jpg")
    resized_img = original_img.resize((600, 500), Image.LANCZOS)
...
def afficher_interface_visiteur():
    ...
    creer_bouton(cadre_droit, "Afficher la map", afficher_plans, 20, 2, '#00ff73','Tw Cen MT',12)
...
def afficher_athlete():
    effacer_label()
    effacer_texte()
    effacer_cadre_milieu()
    global cadre_milieu
    cadre_milieu = tk.Frame(fenetre)
    cadre_milieu.pack(expand=tk.YES, side="top", fill="both")
    cadre_scrollbar = ctk.CTkScrollableFrame(cadre_milieu, label_text="Liste des sportifs", fg_color="white", label_text_color="white")
    cadre_scrollbar.pack(expand=tk.YES, fill=tk.BOTH)

    # On suppose ici que cadre_scrollbar interne est correctement accessible
    # Si ce n'est pas le cas, cette partie du code peut nécessiter des ajustements
    scrollable_frame = cadre_scrollbar  # S'il n'y a pas d'accès direct à un frame interne, utilisez cadre_scrollbar directement

    for lignes in main.admin.show_athlete():
        athlete_info = tk.Label(scrollable_frame, text=lignes, font=("Tw Cent MT", 13))
        athlete_info.pack(pady=10)  # Utiliser pack pour un meilleur gestion du layout
...
def afficher_visiteur():
    ...
    cadre_scrollbar = ctk.CTkScrollableFrame(cadre3,label_text="Liste des sportifs",fg_color="lightgray")
    cadre_scrollbar.pack(expand=tk.YES, fill=tk.BOTH)
    y_position = 50
    for lignes in main.admin.show_visiteur():
        visiteur_info = tk.Label(cadre_scrollbar, text = lignes, bg='#3399FF',font=("Tw Cent MT",13))  # Créer un Label avec les informations du visiteur
